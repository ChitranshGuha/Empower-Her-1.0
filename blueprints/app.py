...
@jobs_bp.route('/choose-skills')
def choose3skills():
    return render_template('choose3skills.html')
...
@jobs_bp.route('/apply_job', methods=['POST'])
def apply_job():
    ...
    try:
        ...
        query = f"SELECT name_of_organization, emailid FROM {skill} WHERE jobid = ?"
    ...
    return redirect(url_for('jobs.email_sent'))

@jobs_bp.route('/email_sent')
def email_sent():
